<template>
  <div>
		<h2 class="subheading grey--text text-decoration-underline">Panel Menu</h2>
    <v-data-table
    ...
    >
      ...
      <template #[`item.menuIcon`]="{ item }">
        <v-icon color="white" :icon="item.menuIcon" />
      </template>
...
export default {
  ...
  computed: {
    ...
    clickrow(event, data) {
      const index = this.$data.expanded.find(i => i === data?.item?.idMenu);
      if(typeof index === 'undefined') return this.$data.expanded = [];
      this.$data.expanded.splice(0, 1)
      this.$data.expanded.push(data?.item?.idMenu);
    },
